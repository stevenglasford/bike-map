...
class VideoValidator:
    """Pre-flight video validation system to detect corrupted and problematic videos"""
    ...
    def __init__(self, config: ProcessingConfig):
        ...
        logger.info(f"Video Validator initialized:")
        logger.info(f"  Quarantine Directory: {self.quarantine_dir}")
    
    def validate_video_batch(self, video_files: List[str], quarantine_corrupted: bool = True) -> Tuple[List[str], List[str], Dict[str, str]]:
        """
        Validate a batch of video files
        
        Returns:
            valid_videos: List of valid video files
            corrupted_videos: List of corrupted video files  
            validation_details: Dict mapping video paths to validation results
        """
        ...
        with tqdm(video_files, desc="Validating videos", unit="video") as pbar:
            for video_path in pbar:
                pbar.set_postfix_str(f"Checking {Path(video_path).name[:30]}...")
                
                validation_result = self._validate_single_video(video_path)
                ...
                if validation_result['is_valid']:
                    ...
                    if quarantine_corrupted:
                        self._quarantine_video(video_path, validation_result['error'])
        
        # Print validation summary
        self._print_validation_summary(valid_videos, corrupted_videos, validation_details)
        
        return valid_videos, corrupted_videos, validation_details
    
    def _validate_single_video(self, video_path: str) -> Dict[str, Any]:
        """Validate a single video file comprehensively"""
        validation_result = {
            'is_valid': False,
            'error': None,
            'file_size_mb': 0,
        ...
        try:
            ...
            if not self._check_video_header(video_path):
                validation_result['error'] = "Invalid video file header"
                return validation_result
            
            # 3. FFprobe validation
            probe_result = self._ffprobe_validation(video_path)
            if not probe_result['success']:
                validation_result['error'] = probe_result['error']
                validation_result['issues'].extend(probe_result.get('issues', []))
                return validation_result
            
            # Update validation result with probe data
            ...
            if validation_result['resolution']:
                ...
                if width < 64 or height < 64:
                    issues.append("Resolution too small")
                if width > 7680 or height > 4320:  # 8K limit
                    issues.append("Resolution extremely high")
            
            # Check for problematic codecs
            codec = validation_result.get('codec', '').lower()
            problematic_codecs = ['prores', 'dnxhd', 'cineform']
            if codec in problematic_codecs:
                issues.append(f"Potentially problematic codec: {codec}")
    ...
    def _check_video_header(self, video_path: str) -> bool:
        """Quick check of video file header for common formats"""
        try:
            ...
            for signature in video_signatures:
                if header.startswith(signature) or signature in header[:32]:
                    return True
            
            # Check for AVI specifically (RIFF...AVI)
    ...
    def _ffprobe_validation(self, video_path: str) -> Dict[str, Any]:
        """Comprehensive FFprobe validation"""
        result = {
            'success': False,
            'error': None,
            'data': {},
        ...
        try:
            # Run ffprobe with comprehensive analysis
            cmd = [
            ...
            if proc_result.returncode != 0:
                ...
                elif 'Truncated packet' in error_output:
                    result['error'] = "Corrupted: Truncated video data"
                else:
                    result['error'] = f"FFprobe error: {error_output[:100]}"
            ...
            # Set resolution tuple
            if result['data']['width'] and result['data']['height']:
                result['data']['resolution'] = (result['data']['width'], result['data']['height'])
            
            result['success'] = True
            return result
            
        except subprocess.TimeoutExpired:
            result['error'] = "FFprobe timeout (file may be corrupted)"
            return result
        except Exception as e:
            result['error'] = f"FFprobe validation failed: {str(e)}"
            return result
    
    def _quarantine_video(self, video_path: str, error_reason: str):
        """Move corrupted video to quarantine directory"""
        try:
            ...
            # Create info file with error details
            ...
            with open(info_path, 'w') as f:
                ...
                f.write(f"File size: {os.path.getsize(quarantine_path) / (1024*1024):.1f} MB\n")
            
            logger.info(f"🚨 Quarantined corrupted video: {video_name} -> {quarantine_path}")
            
        except PermissionError as e:
            logger.error(f"Permission denied when quarantining {video_path}: {e}")
            logger.warning(f"Video remains in original location but will be skipped")
        except OSError as e:
            logger.error(f"OS error when quarantining {video_path}: {e}")
            logger.warning(f"Video remains in original location but will be skipped")
        except Exception as e:
            logger.error(f"Failed to quarantine {video_path}: {e}")
            logger.warning(f"Video remains in original location but will be skipped")
    
    def _print_validation_summary(self, valid_videos: List[str], corrupted_videos: List[str], validation_details: Dict):
        """Print comprehensive validation summary"""
        total_videos = len(valid_videos) + len(corrupted_videos)
        
        print(f"\n{'='*80}")
        print(f"🔍 PRE-FLIGHT VIDEO VALIDATION SUMMARY")
        print(f"{'='*80}")
        ...
        print(f"❌ Corrupted Videos: {len(corrupted_videos)} ({100*len(corrupted_videos)/max(total_videos,1):.1f}%)")
        
        if corrupted_videos:
            print(f"\n🚨 CORRUPTED VIDEOS FOUND:")
            print(f"{'='*80}")
            for video_path in corrupted_videos[:10]:  # Show first 10
                video_name = Path(video_path).name
                error = validation_details[video_path]['error']
                file_size = validation_details[video_path]['file_size_mb']
                print(f"  ❌ {video_name[:50]:<50} | {file_size:6.1f}MB | {error}")
            
            if len(corrupted_videos) > 10:
                print(f"  ... and {len(corrupted_videos) - 10} more corrupted videos")
            
            print(f"\n💡 Corrupted videos have been quarantined to: {self.quarantine_dir}")
        
        if valid_videos:
            print(f"\n✅ VALID VIDEOS SUMMARY:")
            print(f"{'='*80}")
            
            # Calculate statistics
            total_size = sum(validation_details[v]['file_size_mb'] for v in valid_videos)
            total_duration = sum(validation_details[v]['duration'] for v in valid_videos if validation_details[v]['duration'] > 0)
            
            # Codec distribution
            codecs = {}
            resolutions = {}
            for video_path in valid_videos:
                codec = validation_details[video_path].get('codec', 'unknown')
                codecs[codec] = codecs.get(codec, 0) + 1
                
                resolution = validation_details[video_path].get('resolution')
                if resolution:
            ...
            print(f"  Average File Size: {total_size/len(valid_videos):.1f} MB")
            
            # Show top codecs
            if codecs:
                print(f"\n  📹 Video Codecs:")
                for codec, count in sorted(codecs.items(), key=lambda x: x[1], reverse=True)[:5]:
        ...
        # Show videos with issues (but still valid)
        videos_with_issues = [v for v in valid_videos if validation_details[v].get('issues')]
        if videos_with_issues:
            print(f"\n⚠️  VALID VIDEOS WITH POTENTIAL ISSUES:")
            for video_path in videos_with_issues[:5]:
                video_name = Path(video_path).name
                issues = validation_details[video_path]['issues']
                print(f"  ⚠️  {video_name[:50]:<50} | Issues: {', '.join(issues)}")
            if len(videos_with_issues) > 5:
                print(f"  ... and {len(videos_with_issues) - 5} more videos with issues")
    
    def get_validation_report(self, validation_details: Dict) -> Dict:
        """Generate detailed validation report for saving"""
