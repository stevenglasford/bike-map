...
class VideoPreprocessor:
    ...
    def _test_direct_processing(self, video_path: str, gpu_id: int) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.debug(f"ffprobe failed for {Path(video_path).name}: {result.stderr}")
                return False
            
            # Parse the probe result to check for valid streams
            try:
                probe_data = json.loads(result.stdout)
                video_streams = [s for s in probe_data.get('streams', []) if s.get('codec_type') == 'video']
                if not video_streams:
                    logger.debug(f"No video streams found in {Path(video_path).name}")
                    return False
            except json.JSONDecodeError:
                logger.debug(f"Invalid ffprobe output for {Path(video_path).name}")
                return False
            ...
            try:
                ...
                if result.returncode == 0:
                    logger.debug(f"Direct CUDA processing test passed for {Path(video_path).name}")
                    return True
                else:
                    logger.debug(f"Direct CUDA processing test failed for {Path(video_path).name}: {result.stderr.decode()}")
                    return False
    ...
    def _gpu_convert_video(self, video_path: str, gpu_id: int) -> Optional[str]:
        ...
        try:
            # First check if file is completely corrupted
            if not self._is_video_readable(video_path):
                logger.error(f"❌ Video file appears corrupted or unreadable: {Path(video_path).name}")
                return None
            ...
            strategies = [
                ("NVENC Hardware", self._convert_with_nvenc),
                ("CUDA Basic", self._convert_with_cuda_basic),
                ("CPU to GPU-friendly", self._convert_cpu_to_gpu_friendly)
            ]
    ...
    def _is_video_readable(self, video_path: str) -> bool:
        """Check if video file is readable and has basic structure"""
        try:
            ...
            # Quick ffprobe test with more tolerant settings
            cmd = [
                'ffprobe', '-v', 'error', 
                '-select_streams', 'v:0',
            ...
            if result.returncode != 0:
                logger.debug(f"ffprobe basic test failed: {result.stderr}")
                return False
            
            # Check if we got valid output
            output = result.stdout.strip()
            if not output or 'N/A' in output:
                logger.debug(f"ffprobe returned invalid data: {output}")
                return False
            
            return True
            
        except Exception as e:
            logger.debug(f"Video readability check failed: {e}")
            return False
    ...
    def _convert_with_nvenc(self, input_path: str, output_path: Path, gpu_id: int) -> bool:
        ...
        cmd = [
            ...
            '-max_muxing_queue_size', '1024',  # Handle async issues
            str(output_path)
        ]
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.debug(f"NVENC conversion failed: {error_msg[:200]}...")
                return False
        except subprocess.TimeoutExpired:
            logger.debug("NVENC conversion timed out")
            return False
        except Exception as e:
            logger.debug(f"NVENC conversion exception: {e}")
            return False
    ...
    def _convert_with_cuda_basic(self, input_path: str, output_path: Path, gpu_id: int) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                error_msg = result.stderr.decode() if result.stderr else "Unknown error"
                logger.debug(f"CUDA basic conversion failed: {error_msg[:200]}...")
                return False
        except subprocess.TimeoutExpired:
            logger.debug("CUDA basic conversion timed out")
            return False
        except Exception as e:
            logger.debug(f"CUDA basic conversion exception: {e}")
            return False
    ...
    def _convert_cpu_to_gpu_friendly(self, input_path: str, output_path: Path, gpu_id: int) -> bool:
        ...
        if self.config.strict:
            logger.debug("CPU conversion skipped in strict mode")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                error_msg = result.stderr.decode() if result.stderr else "Unknown error"
                logger.debug(f"CPU conversion failed: {error_msg[:200]}...")
                return False
        except subprocess.TimeoutExpired:
            logger.debug("CPU conversion timed out")
            return False
        except Exception as e:
            logger.debug(f"CPU conversion exception: {e}")
            return False
