...
class UltraOptimizedFFmpegDecoder:
    ...
    def __init__(self, gpu_ids=[0, 1], force_software=False):
        ...
        if self.force_software:
            logger.info("Software-only video decoding enabled")
        else:
    ...
    def decode_video_gpu_batch(self, video_path, sample_rate=2.0, target_size=(640, 360), gpu_id=0):
        ...
        # If software decoding is forced, skip GPU entirely
        if self.force_software:
            logger.info("Using software decoding (forced)")
            return self._software_decode_fallback(video_path, sample_rate, target_size, gpu_id)
        
        if gpu_id not in self.temp_dirs:
            logger.warning(f"GPU {gpu_id} not initialized, using software decode")
            return self._software_decode_fallback(video_path, sample_rate, target_size, gpu_id)
        ...
        try:
            ...
            if width >= 3840 or height >= 2160:  # 4K or higher
                ...
                logger.info(f"4K+ video detected ({width}x{height}), using smaller target size {target_size}, rate {sample_rate}")
            elif width >= 1920 or height >= 1080:  # 1080p
                target_size = (640, 360)  # Standard size
                sample_rate = min(sample_rate, 1.5)  # Slight reduction
            
            # Check available GPU memory before proceeding - be more conservative
            ...
            # Need at least 2GB free memory for GPU decoding to be safe
            min_memory_required = 2 * 1024**3  # 2GB
            
            if available_memory < min_memory_required or required_memory > available_memory * 0.5:
                logger.warning(f"Insufficient GPU memory for hardware decode: have {available_memory//1024**2}MB, need {min_memory_required//1024**2}MB minimum")
                logger.info("Using software decode due to memory constraints")
                return self._software_decode_fallback(video_path, sample_rate, target_size, gpu_id)
            
            # Try a simple GPU memory test first
            if not self._test_gpu_decode_capability(gpu_id):
                logger.warning("GPU decoder capability test failed, using software decode")
                return self._software_decode_fallback(video_path, sample_rate, target_size, gpu_id)
            ...
            # Create GPU-accelerated FFmpeg pipeline with very conservative settings
            output_pattern = os.path.join(temp_dir, 'frame_%06d.rgb')
            
            # Try different GPU decoder options in order of preference
            gpu_decoders = ['h264_cuvid', 'hevc_cuvid']  # Remove others that might not work
            
            success = False
            for decoder in gpu_decoders:
                decode_cmd = [
                    'ffmpeg', '-y', '-v', 'error', '-hide_banner',  # Minimal output
                    '-hwaccel', 'cuda',
                ...
                logger.debug(f"Trying GPU decoder: {decoder}")
                
                # Execute with shorter timeout and error checking
                try:
                    process = subprocess.run(decode_cmd, capture_output=True, text=True, timeout=120)  # 2 min timeout
                    if process.returncode == 0:
                        success = True
                        break
                    else:
                        logger.debug(f"GPU decoder {decoder} failed: {process.stderr}")
                        if 'out of memory' in process.stderr.lower() or 'cuda_error_out_of_memory' in process.stderr.lower():
                            logger.warning(f"GPU decoder {decoder} out of memory")
                            break  # Don't try other decoders if OOM
                        # Try next decoder
                        continue
                        
                except subprocess.TimeoutExpired:
                    logger.warning(f"GPU decoder {decoder} timed out")
                    continue
                except Exception as e:
                    logger.debug(f"GPU decoder {decoder} exception: {e}")
                    continue
            
            if not success:
                logger.warning("All GPU decoders failed, using software decode")
                return self._software_decode_fallback(video_path, sample_rate, target_size, gpu_id)
            
            # Load frames directly to GPU
            frame_files = sorted(glob.glob(os.path.join(temp_dir, 'frame_*.rgb')))
            if not frame_files:
                logger.warning("No frames decoded by GPU, using software decode")
                return self._software_decode_fallback(video_path, sample_rate, target_size, gpu_id)
            ...
            logger.info(f"Successfully decoded {len(frames_gpu)} frames using GPU decoder")
            return frames_gpu, fps, duration, frame_indices
            
        except Exception as e:
            logger.error(f"GPU decode failed for {video_path}: {e}")
            # Always try software fallback
            return self._software_decode_fallback(video_path, sample_rate, target_size, gpu_id)
    ...
    def _software_decode_fallback(self, video_path, sample_rate, target_size, gpu_id):
        ...
        if AV_AVAILABLE:
            ...
            # Use OpenCV as ultimate fallback
...
class MaxGPUFeatureExtractor:
    ...
    def extract_all_features_gpu(self, frames_tensor, device_idx=0):
        ...
        if frames_memory > available_memory * 0.7:  # Use max 70% of available memory
            logger.warning(f"Large frame tensor ({frames_memory//1024**2}MB), processing in chunks")
...
class CuPyGPXProcessor:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("CuPy default memory pool limit removed")
        except Exception as e:
        ...
        try:
            ...
            if hasattr(pinned_mempool, 'set_limit'):
                ...
                logger.info("CuPy pinned memory pool limit removed")
            else:
                logger.info("CuPy pinned memory pool set_limit not available in this version")
                
        except Exception as e:
            logger.warning(f"Could not configure CuPy pinned memory pool: {e}")
        
        logger.info("CuPy GPX processor initialized with optimized memory settings")
...
def main():
    ...
    parser.add_argument("--software_decode", action='store_true',
                       help="Force software video decoding (avoids GPU memory issues)")
    ...
    if args.clear_cache:
        logger.info("Clearing all GPU memory and caches...")
        for gpu_id in args.gpu_ids:
            try:
                ...
                with cp.cuda.Device(gpu_id):
                    cp.get_default_memory_pool().free_all_blocks()
                logger.info(f"Cleared GPU {gpu_id}")
            except Exception as e:
                logger.warning(f"Failed to clear GPU {gpu_id}: {e}")
    
    decoder = UltraOptimizedFFmpegDecoder(gpu_ids=args.gpu_ids, force_software=args.software_decode)
    ...
    try:
        ...
        if not video_features or len(video_features) != len(video_files):
            logger.info("Processing videos (cache miss or incomplete)...")
            video_features = {}
            
            # Process videos sequentially by default for better memory management
            logger.info(f"Processing {len(video_files)} videos sequentially to manage GPU memory")
            ...
            for i, video_path in enumerate(tqdm(video_files, desc="Processing videos")):
                ...
                try:
                    # Clear memory before each video
                    torch.cuda.set_device(gpu_id)
                    torch.cuda.empty_cache()
                    ...
                    features = process_video_ultra_optimized_with_settings(
                    ...
                    if features is None:
                        ...
                        logger.debug(f"Successfully processed {video_path} ({i+1}/{len(video_files)})")
                ...
                if (i + 1) % 10 == 0:
                    ...
                    logger.info(f"Saved progress: {i+1}/{len(video_files)} videos processed")
            
            # Final cache save
            with open(video_cache_path, 'wb') as f:
                pickle.dump(video_features, f)
            
            valid_videos = sum(1 for v in video_features.values() if v is not None)
            logger.info(f"Ultra-processed and cached {valid_videos}/{len(video_features)} videos successfully")
