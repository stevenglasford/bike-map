...
class EnhancedFFmpegDecoder:
    ...
    def _decode_uniform_frames(self, video_path: str, video_info: Dict, gpu_id: int) -> Optional[torch.Tensor]:
        ...
        if total_frames > max_frames:
            ...
            # Simplified filter chain without CUDA output format
        ...
        cuda_cmd = [
            ...
            '-q:v', '3',  # Slightly lower quality for memory efficiency
            '-threads', '1',  # Limit threads for memory
            output_pattern
        ]
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"CUDA FFmpeg successful for {Path(video_path).name}")
            except subprocess.CalledProcessError as e:
                error_output = e.stderr.decode() if e.stderr else "Unknown FFmpeg error"
                if ("Impossible to convert between the formats" in error_output or 
                    "Function not implemented" in error_output or
                    "CUDA out of memory" in error_output):
                    logger.debug(f"CUDA FFmpeg failed, trying CPU fallback: {error_output[:100]}...")
                    
                    if self.config.strict:
                        raise RuntimeError(f"STRICT MODE: CUDA FFmpeg failed and CPU fallback not allowed: {error_output}")
