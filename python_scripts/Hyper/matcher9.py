...
def process_video_ultra_optimized(video_path, decoder, feature_extractor, gpu_idx, target_size, sample_rate):
    """Process single video with STRICT GPU acceleration - no CPU fallbacks"""
    try:
        # Clear GPU memory before processing
        ...
        # Also clear CuPy memory
        with cp.cuda.Device(gpu_idx):
            cp.get_default_memory_pool().free_all_blocks()
        
        logger.debug(f"STRICT MODE: Processing {video_path} on GPU {gpu_idx} with size {target_size}, rate {sample_rate}")
        
        # Check available memory
        available_memory = torch.cuda.get_device_properties(gpu_idx).total_memory - torch.cuda.memory_allocated(gpu_idx)
        logger.debug(f"STRICT MODE: Available GPU memory: {available_memory // 1024**2}MB")
        
        if available_memory < 512 * 1024**2:  # Less than 512MB
            raise RuntimeError(f"STRICT MODE: Insufficient GPU memory: {available_memory // 1024**2}MB available")
        
        # Decode with STRICT GPU acceleration - NO FALLBACKS
        ...
        if frames_tensor is None:
            raise RuntimeError(f"STRICT MODE: GPU video decoding failed for {video_path}")
        
        # STRICT: Validate frames are on GPU
        StrictGPUEnforcer.enforce_tensor_on_gpu(frames_tensor, "decoded video frames")
        
        logger.debug(f"STRICT MODE: Decoded {frames_tensor.shape} frames on GPU")
        
        # Extract features with STRICT GPU utilization
        features = feature_extractor.extract_all_features_gpu(frames_tensor, gpu_idx)
        
        # STRICT: Validate all features are computed
        if not features:
            raise RuntimeError(f"STRICT MODE: No features extracted for {video_path}")
        
        # Convert GPU tensors to numpy for storage (but validate they were on GPU first)
        for key, value in features.items():
            if isinstance(value, torch.Tensor):
                StrictGPUEnforcer.enforce_tensor_on_gpu(value, f"feature {key}")
        ...
        features['gpu_processed'] = True  # Mark as GPU processed
        
        # Clean up GPU memory immediately after processing
        del frames_tensor
        torch.cuda.empty_cache()
        ...
        logger.debug(f"STRICT MODE: Successfully processed {video_path} using GPU acceleration")
        return features
        
    except Exception as e:
        logger.error(f"STRICT MODE: GPU processing failed for {video_path}: {e}")
        ...
        raise RuntimeError(f"STRICT MODE: GPU acceleration failed for {video_path}: {e}")
