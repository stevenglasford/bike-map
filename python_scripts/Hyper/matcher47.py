...
class SharedGPUResourceManager:
    """NEW: Shared GPU resource manager to eliminate redundant object creation"""
    
    def __init__(self, gpu_manager, config: CompleteTurboConfig):
        self.gpu_manager = gpu_manager
        self.config = config
        
        # Create shared extractors per GPU (reused across all videos)
        self.shared_optical_flow_extractors = {}
        self.shared_cnn_extractors = {}
        self.shared_processors = {}
        
        # Initialize shared resources on each GPU
        for gpu_id in gpu_manager.gpu_ids:
            if config.use_optical_flow:
                self.shared_optical_flow_extractors[gpu_id] = Enhanced360OpticalFlowExtractor(config)
            
            if config.use_pretrained_features or config.enable_spherical_processing:
                self.shared_cnn_extractors[gpu_id] = Enhanced360CNNFeatureExtractor(gpu_manager, config)
        
        logger.info(f"🚀 Shared GPU Resource Manager initialized - eliminates redundant object creation")
    
    def get_optical_flow_extractor(self, gpu_id: int):
        """Get shared optical flow extractor for GPU"""
        return self.shared_optical_flow_extractors.get(gpu_id)
    
    def get_cnn_extractor(self, gpu_id: int):
        """Get shared CNN extractor for GPU"""
        return self.shared_cnn_extractors.get(gpu_id)
...
class TurboGPUManager:
    ...
    def __init__(self, gpu_ids: List[int], strict: bool = False, config: Optional[CompleteTurboConfig] = None):
        ...
        # Initialize GPU queue with all GPUs
...
class OptimizedVideoProcessor:
    """FIXED: Optimized video processor using shared resources"""
    
    def __init__(self, shared_resources: SharedGPUResourceManager, config: CompleteTurboConfig):
        self.shared_resources = shared_resources
        self.config = config
        self.gpu_manager = shared_resources.gpu_manager
        
        # Create temp directories per GPU (shared across all instances)
        base_temp = Path(config.cache_dir) / "optimized_temp"
        base_temp.mkdir(parents=True, exist_ok=True)
        
        self.temp_dirs = {}
        for gpu_id in shared_resources.gpu_manager.gpu_ids:
            self.temp_dirs[gpu_id] = base_temp / f'gpu_{gpu_id}'
            self.temp_dirs[gpu_id].mkdir(exist_ok=True)
        
        logger.debug("Optimized video processor initialized with shared resources")
    
    def process_video_optimized(self, video_path: str) -> Optional[Dict]:
        """FIXED: Process video using shared resources - no redundant object creation"""
        gpu_id = self.gpu_manager.acquire_gpu(timeout=self.config.gpu_timeout)
        
        if gpu_id is None:
            error_msg = f"Could not acquire GPU for {Path(video_path).name}"
        ...
        try:
            # FIXED: Use shared resources instead of creating new ones
            optical_flow_extractor = self.shared_resources.get_optical_flow_extractor(gpu_id)
            cnn_extractor = self.shared_resources.get_cnn_extractor(gpu_id)
            ...
            if stream:
                with torch.cuda.stream(stream):
                    return self._extract_features_with_shared_resources(
                        video_path, gpu_id, optical_flow_extractor, cnn_extractor
                    )
            else:
                return self._extract_features_with_shared_resources(
                    video_path, gpu_id, optical_flow_extractor, cnn_extractor
    ...
    def _extract_features_with_shared_resources(self, video_path: str, gpu_id: int, 
                                              optical_flow_extractor, cnn_extractor) -> Optional[Dict]:
        """FIXED: Extract features using shared resources"""
        try:
            # Decode video with optimized settings
            frames_tensor, fps, duration = self._decode_video_optimized(video_path, gpu_id)
            
            if frames_tensor is None:
                return None
            
            # Extract features using shared extractors (no object creation)
            features = self._extract_all_features_shared(
                frames_tensor, gpu_id, optical_flow_extractor, cnn_extractor
            )
            ...
            features['processing_mode'] = 'OptimizedShared_GPU'
            ...
            logger.debug(f"Optimized feature extraction failed for {video_path}: {e}")
            return None
...
class CompleteTurboVideoProcessor:
    ...
    def _decode_video_optimized(self, video_path: str, gpu_id: int) -> Tuple[Optional[torch.Tensor], float, float]:
        """FIXED: Optimized video decoding with shared temp directories"""
        try:
            # Get video info quickly
            video_info = self._get_video_info_fast(video_path)
            if not video_info:
            ...
            temp_dir = self.temp_dirs[gpu_id]
            output_pattern = os.path.join(temp_dir, f'frame_{gpu_id}_{int(time.time())}_%06d.jpg')
            ...
            # FIXED: Optimized ffmpeg command with shorter timeout
            cmd = [
                'ffmpeg', '-y', '-v', 'error', '-hide_banner',
                '-hwaccel', 'cuda', '-hwaccel_device', str(gpu_id),
                '-i', video_path,
            ...
            # FIXED: Much shorter timeout to prevent blocking
            timeout = 30 if self.config.turbo_mode else 60
            result = subprocess.run(cmd, check=True, capture_output=True, timeout=timeout)
            
            return self._load_frames_optimized(temp_dir, gpu_id, target_width, target_height, int(time.time()))
            
        except Exception as e:
            logger.debug(f"Optimized decoding failed for {video_path}: {e}")
            return None, 0, 0
    
    def _get_video_info_fast(self, video_path: str) -> Optional[Dict]:
        """FIXED: Fast video info with aggressive timeout"""
        try:
            cmd = ['ffprobe', '-v', 'quiet', '-print_format', 'json', 
                   '-show_format', '-show_streams', video_path]
            
            # FIXED: Very short timeout
    ...
    def _load_frames_optimized(self, temp_dir: str, gpu_id: int, width: int, height: int, timestamp: int) -> Optional[torch.Tensor]:
        """FIXED: Optimized frame loading with immediate cleanup"""
        frame_files = sorted(glob.glob(os.path.join(temp_dir, f'frame_{gpu_id}_{timestamp}_*.jpg')))
    ...
    def _extract_all_features_shared(self, frames_tensor: torch.Tensor, gpu_id: int, 
                                   optical_flow_extractor, cnn_extractor) -> Dict[str, np.ndarray]:
        """FIXED: Extract features using shared extractors"""
        try:
            device = torch.device(f'cuda:{gpu_id}')
            ...
            motion_features = self._compute_basic_motion(frames_tensor[0], device)
            features.update(motion_features)
            
            # Optical flow features using shared extractor
            if optical_flow_extractor and self.config.use_optical_flow:
                try:
                    optical_flow_features = optical_flow_extractor.extract_optical_flow_features(frames_tensor, gpu_id)
                    features.update(optical_flow_features)
                except Exception as e:
                    logger.debug(f"Optical flow extraction failed: {e}")
            
            # CNN features using shared extractor
            if cnn_extractor:
                try:
                    cnn_features = cnn_extractor.extract_enhanced_features(frames_tensor, gpu_id)
                    features.update(cnn_features)
                except Exception as e:
                    logger.debug(f"CNN feature extraction failed: {e}")
            
            # Basic color and edge features
            color_features = self._compute_basic_color(frames_tensor[0], device)
            features.update(color_features)
            
            edge_features = self._compute_basic_edges(frames_tensor[0], device)
            features.update(edge_features)
    ...
    def _compute_basic_motion(self, frames: torch.Tensor, device: torch.device) -> Dict[str, np.ndarray]:
        ...
        for i in range(num_frames - 1):
            diff = torch.abs(gray_frames[i + 1] - gray_frames[i])
            magnitude = torch.mean(diff).item()
            features['motion_magnitude'][i + 1] = magnitude
    ...
    def _compute_basic_color(self, frames: torch.Tensor, device: torch.device) -> Dict[str, np.ndarray]:
    ...
    def _compute_basic_edges(self, frames: torch.Tensor, device: torch.device) -> Dict[str, np.ndarray]:
        """Basic edge computation"""
        # Simple edge detection
        gray_frames = 0.299 * frames[:, 0] + 0.587 * frames[:, 1] + 0.114 * frames[:, 2]
        
        # Gradient-based edge detection
        edges_x = torch.abs(torch.diff(gray_frames, dim=2))
        edges_y = torch.abs(torch.diff(gray_frames, dim=1))
        
        edge_density_x = torch.mean(edges_x, dim=[1, 2]).cpu().numpy()
        edge_density_y = torch.mean(edges_y, dim=[1, 2]).cpu().numpy()
        edge_density = (edge_density_x + edge_density_y) / 2
        
        return {
            'edge_density': edge_density
        }

def process_video_parallel_optimized(args) -> Tuple[str, Optional[Dict]]:
    """FIXED: Optimized video processing using shared resources"""
    video_path, shared_resources, powersafe_manager = args
    ...
    try:
        # FIXED: Use optimized processor with shared resources
        processor = OptimizedVideoProcessor(shared_resources, shared_resources.config)
        
        # Process video using shared resources
        features = processor.process_video_optimized(video_path)
        
        if features is None:
            error_msg = f"Video processing failed for {Path(video_path).name}"
            
            if shared_resources.config.strict_fail:
                error_msg = f"ULTRA STRICT MODE: {error_msg}"
                if powersafe_manager:
                    powersafe_manager.mark_video_failed(video_path, error_msg)
                raise RuntimeError(error_msg)
            elif shared_resources.config.strict:
        ...
        if shared_resources.config.turbo_mode:
            success_msg += " [OPTIMIZED]"
