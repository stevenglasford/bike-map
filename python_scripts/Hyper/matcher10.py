...
class UltraOptimizedFFmpegDecoder:
    ...
    def __init__(self, gpu_ids=[0, 1], skip_decode_test=False):
    ...
    def _test_gpu_decode_capability(self, gpu_id):
        ...
        try:
            ...
            # Test 4: FFmpeg with CUDA (the actual test)
            cuda_test_cmd = [
                'ffmpeg', '-y', '-v', 'error', '-hide_banner',
                '-f', 'lavfi', '-i', 'color=red:size=320x240:duration=0.1',
...
def main():
    ...
    decoder = UltraOptimizedFFmpegDecoder(gpu_ids=args.gpu_ids, skip_decode_test=args.skip_decode_test)
