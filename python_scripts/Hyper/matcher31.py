...
class EnhancedFFmpegDecoder:
    ...
    def _decode_uniform_frames(self, video_path: str, video_info: Dict, gpu_id: int) -> Optional[torch.Tensor]:
        """Memory-optimized uniform frame sampling with robust FFmpeg handling"""
        ...
        if self.config.memory_efficient:
            ...
            mem_info = gpu_manager.get_gpu_memory_info(gpu_id)
            if mem_info['free_gb'] < 2.0:
                # Reduce frame count if low memory
        ...
        if total_frames > max_frames:
            ...
            # CUDA-aware filter chain - keep everything on GPU
            vf_filter = f'scale_cuda={target_width}:{target_height}:force_original_aspect_ratio=decrease:force_divisible_by=2,pad_cuda={target_width}:{target_height}:(ow-iw)/2:(oh-ih)/2,hwdownload,format=nv12,select=not(mod(n\\,{int(sample_rate)}))'
        else:
            vf_filter = f'scale_cuda={target_width}:{target_height}:force_original_aspect_ratio=decrease:force_divisible_by=2,pad_cuda={target_width}:{target_height}:(ow-iw)/2:(oh-ih)/2,hwdownload,format=nv12'
        
        # Proper CUDA-accelerated FFmpeg command
        cuda_cmd = [
            'ffmpeg', '-y', '-v', 'error', '-hide_banner',
            '-hwaccel', 'cuda', '-hwaccel_device', str(gpu_id),
            '-hwaccel_output_format', 'cuda',  # Keep decoded frames on GPU
            '-i', video_path,
            '-vf', vf_filter,  # Use CUDA filters throughout
        ...
        # Fallback CUDA command with simpler filter chain
        cuda_simple_cmd = [
            'ffmpeg', '-y', '-v', 'error', '-hide_banner',
            '-hwaccel', 'cuda', '-hwaccel_device', str(gpu_id),
            '-i', video_path,
            '-vf', f'scale_cuda={target_width}:{target_height},hwdownload,format=yuv420p' + (f',select=not(mod(n\\,{int(sample_rate)}))' if total_frames > max_frames else ''),
            '-frames:v', str(min(max_frames, total_frames)),
            '-q:v', '3',
            '-pix_fmt', 'yuv420p',
            output_pattern
        ...
        # CPU fallback command (only as last resort)
        cpu_cmd = [
            'ffmpeg', '-y', '-v', 'error', '-hide_banner',
            '-i', video_path,
            '-vf', f'scale={target_width}:{target_height}:force_original_aspect_ratio=decrease:force_divisible_by=2,pad={target_width}:{target_height}:(ow-iw)/2:(oh-ih)/2' + (f',select=not(mod(n\\,{int(sample_rate)}))' if total_frames > max_frames else ''),
            '-frames:v', str(min(max_frames, total_frames)),
            '-q:v', '3',
            '-threads', '2',
            output_pattern
        ...
        try:
            ...
            mem_before = gpu_manager.get_gpu_memory_info(gpu_id)
            logger.debug(f"GPU {gpu_id} memory before FFmpeg: {mem_before['free_gb']:.1f}GB free")
            
            # Try CUDA commands in order of preference
            success = False
            
            # First try: Full CUDA pipeline with advanced filters
            try:
                result = subprocess.run(cuda_cmd, check=True, capture_output=True, timeout=300)
                logger.debug(f"Advanced CUDA FFmpeg successful for {Path(video_path).name}")
                success = True
            except subprocess.CalledProcessError as e:
                error_output = e.stderr.decode() if e.stderr else "Unknown FFmpeg error"
                logger.debug(f"Advanced CUDA failed: {error_output[:150]}...")
            ...
            # Second try: Simplified CUDA pipeline
            if not success:
                try:
                    result = subprocess.run(cuda_simple_cmd, check=True, capture_output=True, timeout=300)
                    logger.debug(f"Simple CUDA FFmpeg successful for {Path(video_path).name}")
                    success = True
                except subprocess.CalledProcessError as e:
                    error_output = e.stderr.decode() if e.stderr else "Unknown FFmpeg error"
                    logger.debug(f"Simple CUDA failed: {error_output[:150]}...")
            ...
            # Last resort: CPU fallback (only if not in strict mode or for memory issues)
            if not success:
                if self.config.strict:
                    raise RuntimeError(f"STRICT MODE: All CUDA FFmpeg methods failed for {video_path}")
                
                logger.debug(f"Trying CPU fallback for {Path(video_path).name}")
                try:
                    result = subprocess.run(cpu_cmd, check=True, capture_output=True, timeout=300)
                    logger.debug(f"CPU FFmpeg fallback successful for {Path(video_path).name}")
                    success = True
                except subprocess.CalledProcessError as e:
                    error_output = e.stderr.decode() if e.stderr else "Unknown FFmpeg error"
                    logger.error(f"All FFmpeg methods failed for {video_path}: {error_output}")
                    return None
            
            if not success:
                logger.error(f"All FFmpeg methods failed for {video_path}")
                return None
            
            # Check memory after
            mem_after = gpu_manager.get_gpu_memory_info(gpu_id)
            logger.debug(f"GPU {gpu_id} memory after FFmpeg: {mem_after['free_gb']:.1f}GB free")
            ...
            error_output = e.stderr.decode() if e.stderr else "Unknown FFmpeg error"
            logger.error(f"FFmpeg failed for {video_path}: {error_output}")
            return None
        except Exception as e:
