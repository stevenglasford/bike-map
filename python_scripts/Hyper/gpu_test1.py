...
class ComprehensiveGPUTester:
    ...
    def test_video_decoding_gpu(self) -> Dict:
        ...
        try:
            ...
            # Look for the main script in current directory
            main_script_path = Path('./gpu_optimized_matcher.py')
            if not main_script_path.exists():
                main_script_path = Path('./matcher.py')
            
            if main_script_path.exists():
                spec = importlib.util.spec_from_file_location("gpu_optimized_matcher", main_script_path)
                gpu_module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(gpu_module)
                
                decoder = gpu_module.UltraOptimizedFFmpegDecoder(gpu_ids=self.gpu_ids)
            else:
                raise ImportError("Main GPU script not found")
    ...
    def test_feature_extraction_gpu(self) -> Dict:
        ...
        try:
            ...
            if main_script_path.exists():
                ...
                extractor = gpu_module.MaxGPUFeatureExtractor(gpu_ids=self.gpu_ids)
            else:
                raise ImportError("Main GPU script not found")
    ...
    def test_gpx_processing_gpu(self) -> Dict:
        ...
        try:
            ...
            if main_script_path.exists():
                ...
                processor = gpu_module.CuPyGPXProcessor()
            else:
                raise ImportError("Main GPU script not found")
    ...
    def test_correlation_gpu(self) -> Dict:
        ...
        try:
            ...
            if main_script_path.exists():
                ...
                correlator = gpu_module.UltraHighPerformanceCorrelator(gpu_ids=self.gpu_ids)
            else:
                raise ImportError("Main GPU script not found")
    ...
    def test_gpu_memory_allocation(self) -> Dict:
        ...
        for gpu_id in self.gpu_ids:
            ...
            # Cleanup every other fragment (create new list to avoid index issues)
            fragments_to_keep = [fragments[i] for i in range(len(fragments)) if i % 2 == 1]
            fragments_to_delete = [fragments[i] for i in range(len(fragments)) if i % 2 == 0]
