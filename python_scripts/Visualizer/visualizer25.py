...
class SimplifiedGPUYOLOProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Processing settings
        self.batch_size = config.get('batch_size', 8)  # Conservative batch size
    ...
    def _initialize_gpus_simple(self):
        ...
        for gpu_id in range(self.gpu_count):
            ...
            # Simple device setup (like your working scripts)
            device = "cuda" if torch.cuda.is_available() else "cpu"
            torch_device = torch.device(f'cuda:{gpu_id}')
            self.devices[gpu_id] = torch_device
    ...
    def _load_yolo_model_simple(self, gpu_id: int):
        """Simple YOLO model loading (like your working process*.py scripts)"""
        ...
        try:
            # Simple YOLO initialization (exactly like your working scripts)
    ...
    def process_video_simple_gpu(self, video_path: str, gpu_id: int, gps_df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # Process frames in batches (simple approach)
            for i in range(0, len(frames_data), self.batch_size):
                ...
                # Process each frame in batch (like your working scripts)
                for frame, frame_idx in batch_frames:
                    
                    # Simple YOLO inference (exactly like your working scripts)
                    ...
                    detection_data = self._extract_detections_simple(
                        results, frame_idx, gpu_id, video_info
                    )
                ...
                if batch_idx % 10 == 0:
                    progress = ((i + len(batch_frames)) / len(frames_data)) * 100
                    batch_time = time.time() - batch_start
                    fps = len(batch_frames) / batch_time
    ...
    def _extract_detections_simple(self, results, frame_idx: int, 
                                  gpu_id: int, video_info: Dict) -> Dict:
        """Extract detections (simple approach like your working scripts)"""
        detection_data = {
            'frame_idx': frame_idx,
            'detections': [],
        ...
        # Handle YOLO results (simple approach)
