...
# Simple imports (like your working scripts)
try:
    import torch
...
def detect_num_gpus():
    """Detect number of GPUs (like your unified.py)"""
    try:
        import subprocess
        output = subprocess.check_output(["nvidia-smi", "-L"]).decode("utf-8")
        return len(output.strip().splitlines())
    except Exception as e:
        logger.warning(f"Could not detect GPUs with nvidia-smi: {e}")
        return 1
...
def process_video_single_gpu(video_path: str, gps_df: pd.DataFrame, 
    ...
    try:
        ...
        # Process frames - EXACTLY like your working scripts
        ...
        logger.info(f"🔥 Starting GPU inference...")
        
        # EXACTLY like your working process*.py scripts - frame by frame
        for frame_idx in range(0, frame_count, frame_skip):
            ...
            if processed_frames % 100 == 0:
                ...
                logger.info(f"🔥 Progress: {progress:.1f}% - {fps_current:.1f} FPS - Frame {frame_idx}/{frame_count}")
