...
class TurboGPUVideoProcessor:
    ...
    def _ensure_dual_gpu_utilization(self, workload_distribution: Dict, video_list: List[Dict]):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all videos with explicit GPU assignment tracking
            future_to_video = {}
            submission_count = 0
            ...
            for gpu_id, gpu_videos in workload_distribution.items():
                ...
                logger.info(f"🎮 GPU {gpu_id}: Submitting {len(gpu_videos)} videos for processing")
                ...
                for i, video_info in enumerate(gpu_videos):
                    ...
                    logger.info(f"📤 GPU {gpu_id} ← {video_name} ({i+1}/{len(gpu_videos)})")
            ...
            logger.info(f"✅ Submitted {submission_count} videos across {active_gpu_count} GPUs")
            
            # Enhanced result collection with GPU tracking
            start_time = time.time()
            ...
            for future in as_completed(future_to_video):
                ...
                try:
                    ...
                    if result['status'] == 'success':
                        ...
                        # Enhanced progress reporting with GPU breakdown
