...
Author: AI Assistant
Target: Guaranteed GPU acceleration for YOLO
"""
...
class GPUUsageVerifier:
    ...
    def record_baseline(self, gpu_id: int):
        ...
        if torch.cuda.is_available():
            ...
            self.baseline_memory[gpu_id] = torch.cuda.memory_allocated(gpu_id) / (1024**3)
            logger.info(f"📊 GPU {gpu_id} baseline memory: {self.baseline_memory[gpu_id]:.2f}GB")
    ...
    def verify_gpu_usage(self, gpu_id: int) -> bool:
        ...
        if torch.cuda.is_available():
            ...
            # GPU is being used if memory increased significantly
            is_using_gpu = memory_increase > 0.5  # At least 500MB increase
            
            if is_using_gpu:
                logger.info(f"✅ GPU {gpu_id}: CONFIRMED GPU processing (memory increased)")
            else:
                logger.error(f"❌ GPU {gpu_id}: NO GPU usage detected (insufficient memory increase)")
...
class VerifiedGPUYOLOProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_video_frames = 6000  # Reasonable limit
    ...
    def _initialize_and_verify_gpus(self):
        """Initialize GPUs with explicit verification"""
        ...
        for gpu_id in self.gpu_ids:
            ...
            if not self.gpu_verifier.verify_gpu_usage(gpu_id):
                raise RuntimeError(f"❌ GPU {gpu_id} verification FAILED - not using GPU!")
            
            logger.info(f"✅ GPU {gpu_id} VERIFIED and ready for processing")
        
        # Final verification test
        self._run_gpu_verification_test()
        
        logger.info(f"🎉 ALL {self.gpu_count} GPUs VERIFIED and ready!")
    ...
    def _load_and_verify_yolo_model(self, gpu_id: int):
        ...
        try:
            ...
            # Test model inference to ensure GPU usage
            self._test_model_gpu_inference(gpu_id)
    ...
    def _test_model_gpu_inference(self, gpu_id: int):
        """Test model inference to verify GPU usage"""
        ...
        test_tensor = torch.randn(1, 3, 640, 640, device=device)
        ...
        logger.info(f"✅ GPU {gpu_id} inference test successful ({inference_time:.3f}s)")
    ...
    def extract_frames_for_gpu(self, video_path: str) -> Tuple[np.ndarray, Dict]:
        """Extract frames optimized for GPU processing"""
        logger.info(f"📹 Extracting frames for GPU: {Path(video_path).name}")
        ...
        # Frame selection for GPU processing
        max_frames = min(self.max_video_frames, frame_count)
        
        # Calculate frame skip
        if frame_count > max_frames:
            frame_skip = max(self.frame_skip, frame_count // max_frames)
        else:
            frame_skip = self.frame_skip
        
        frame_indices = list(range(0, frame_count, frame_skip))[:max_frames]
        
        logger.info(f"📊 GPU processing: {len(frame_indices)} frames from {frame_count} total")
    ...
    def process_video_verified_gpu(self, video_path: str, gpu_id: int, gps_df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            for batch_idx, (batch_frames, frame_indices) in enumerate(dataloader):
                ...
                if memory_increase < 0.1:  # Less than 100MB increase
                    logger.warning(f"⚠️ GPU {gpu_id} low memory usage during processing")
            ...
            if utilization_log:
                avg_utilization = np.mean(utilization_log)
    ...
    def process_videos_verified_dual_gpu(self, video_matches: Dict[str, Any]):
        ...
        for i in range(0, len(video_list), self.gpu_count):
            ...
            for video_path, result in results.items():
                if result and result['status'] == 'success':
                    ...
                    avg_util = np.mean(gpu_util) if gpu_util else 0
        ...
        if self.stats['gpu_utilization_log']:
            avg_gpu_util = np.mean(self.stats['gpu_utilization_log'])
    ...
    def _generate_verification_report(self, total_time: float):
        ...
        report = {
            'gpu_verification_summary': {
                ...
                'average_gpu_utilization': np.mean(self.stats['gpu_utilization_log']) if self.stats['gpu_utilization_log'] else 0
            },
            'gpu_details': {
