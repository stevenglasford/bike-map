...
class TurboGPUVideoProcessor:
    ...
    def process_videos_dual_gpu_turbo(self, video_matches: Dict[str, Any]):
        ...
        if len(self.gpu_ids) == 2 and (total_videos > 1 or self.config.get('force_dual_gpu', False)):
            workload_distribution = self._ensure_dual_gpu_utilization(workload_distribution, video_list)
        
        # Calculate active GPU count AFTER distribution is finalized
        ...
        if active_gpu_count < len(self.gpu_ids) and total_videos > 1:
            logger.warning(f"⚠️ Suboptimal GPU usage: Only {active_gpu_count}/{len(self.gpu_ids)} GPUs will be used")
        ...
        # NOW actually process the videos
        logger.info(f"🔄 Starting processing with {active_gpu_count} active GPUs")
        self._process_dual_gpu_workload(workload_distribution, total_videos, gpu_stats, active_gpu_count)
    
    def _ensure_dual_gpu_utilization(self, workload_distribution: Dict, video_list: List[Dict]) -> Dict:
        """Ensure both GPUs get work when we have multiple videos or forced dual GPU - RETURNS the updated distribution"""
    ...
    def _process_dual_gpu_workload(self, workload_distribution: Dict, total_videos: int, gpu_stats: Dict, active_gpu_count: int):
        """Process workload across dual GPUs with true parallelism"""
        # Create result queue for collecting results
        result_queue = queue.Queue()
        completed_videos = 0
        
        # Create worker threads for each GPU that has work
        worker_threads = []
        
        for gpu_id, gpu_videos in workload_distribution.items():
            if not gpu_videos:  # Skip GPUs with no videos
                logger.info(f"🎮 GPU {gpu_id}: No videos assigned, skipping")
                continue
        ...
        for thread in worker_threads:
            ...
            if thread.is_alive():
                logger.warning(f"⚠️ Worker thread {thread.name} still alive after timeout")
    ...
    def _gpu_worker_thread(self, gpu_id: int, gpu_videos: List[Dict], result_queue: queue.Queue):
        """Enhanced worker thread that processes videos on a specific GPU with proper error handling"""
        ...
        try:
            ...
            # Verify GPU context is set correctly
            current_device = torch.cuda.current_device()
            if current_device != gpu_id:
                logger.warning(f"⚠️ GPU {gpu_id} context mismatch: expected {gpu_id}, got {current_device}")
                torch.cuda.set_device(gpu_id)
            
            # Verify model and memory manager are accessible
            if gpu_id not in self.models:
                raise RuntimeError(f"GPU {gpu_id} model not initialized")
            if gpu_id not in self.gpu_managers:
                raise RuntimeError(f"GPU {gpu_id} memory manager not initialized")
            
            logger.info(f"✅ GPU {gpu_id} worker: Context verified, processing {len(gpu_videos)} videos")
            ...
            for i, video_info in enumerate(gpu_videos):
                ...
                try:
                    ...
                    if not os.path.exists(video_info['path']):
                        raise FileNotFoundError(f"Video file not found: {video_info['path']}")
                    
                    # Load GPS data
                    gps_df = self._load_gps_data_fast(video_info['gps_path'])
                    
                    # Process video on this GPU with error handling
                    result = self.process_video_turbo(video_info['path'], gpu_id, gps_df)
                    
                    # Verify result format
                    if not isinstance(result, dict) or 'status' not in result:
                        raise ValueError(f"Invalid result format from process_video_turbo")
                    
                    # Add video name if missing
                    ...
                    if result['status'] == 'success':
                        ...
                        logger.info(f"✅ GPU {gpu_id} completed: {video_name} ({fps:.1f} FPS)")
                    else:
                        logger.warning(f"⚠️ GPU {gpu_id} finished with status: {result['status']}")
                    
                except Exception as e:
                    logger.error(f"❌ GPU {gpu_id} error processing {video_name}: {str(e)}")
                    ...
                    try:
                        torch.cuda.empty_cache()
                        logger.info(f"🔧 GPU {gpu_id} cache cleared after error")
                    except Exception as cleanup_error:
                        logger.error(f"❌ GPU {gpu_id} cleanup failed: {cleanup_error}")
            
            logger.info(f"✅ GPU {gpu_id} worker thread completed all {len(gpu_videos)} videos")
            
        except Exception as e:
            logger.error(f"❌ GPU {gpu_id} worker thread critical failure: {str(e)}")
            logger.error(f"   Worker will terminate. Error type: {type(e).__name__}")
            ...
            for video_info in gpu_videos:
                ...
                error_result = {
                    'status': 'failed',
                    'error': f"Worker thread critical failure: {type(e).__name__}: {str(e)}",
                    'gpu_id': gpu_id,
                    'video_name': video_name,
                    'video_path': video_info['path']
                }
            ...
            try:
                torch.cuda.empty_cache()
                logger.info(f"🧹 GPU {gpu_id} worker: Final cleanup completed")
            except Exception as cleanup_error:
                logger.error(f"❌ GPU {gpu_id} final cleanup error: {cleanup_error}")
...
Examples:
  ...
  python visualizer.py -i results.json -o output/
  
  # Maximum performance with custom batch size
  python visualizer.py -i results.json -o output/ --batch-size 1024
  
  # High-quality filtering with performance monitoring
  python visualizer.py -i results.json -o output/ --min-score 0.8 --detailed-profiling
        """
    )
