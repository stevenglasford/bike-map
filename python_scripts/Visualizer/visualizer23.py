...
class VerifiedGPUDataset(Dataset):
    """Dataset that ensures GPU tensor creation and verification"""
    
    def __init__(self, frames: np.ndarray, device: torch.device):
        self.frames = frames
        self.device = device
        logger.info(f"📦 Dataset created for device: {device}")
        logger.info(f"📊 Dataset frames shape: {frames.shape}")
    
    def __len__(self):
        return len(self.frames)
    
    def __getitem__(self, idx):
        # Create tensor and FORCE to GPU device
        frame = torch.from_numpy(self.frames[idx])
        
        # CRITICAL: Ensure tensor is on correct GPU
        ...
        if frame.device.type != 'cuda':
            raise RuntimeError(f"Frame tensor not on GPU! Device: {frame.device}")
...
class VerifiedGPUYOLOProcessor:
    ...
    def _load_and_verify_yolo_model(self, gpu_id: int):
        ...
        try:
            # Load YOLO model with explicit device specification
    ...
    def _test_model_gpu_inference(self, gpu_id: int):
        ...
        with torch.no_grad():
            ...
            # Force GPU context and synchronization
            torch.cuda.set_device(gpu_id)
            torch.cuda.synchronize(gpu_id)
            
            # Test YOLO inference with proper device context
            results = model(test_tensor, verbose=False, device=gpu_id)
