...
def create_balanced_gpu_queues(all_matches, gpu_ids=[0, 1]):
    """Create separate queues for each GPU with pre-balanced work"""
    
    # Create dedicated queues for each GPU
    gpu_queues = {gpu_id: queue.Queue() for gpu_id in gpu_ids}
    
    # Shuffle matches to avoid any ordering bias
    shuffled_matches = list(all_matches)
    random.shuffle(shuffled_matches)
    
    # Round-robin distribution
    for i, match in enumerate(shuffled_matches):
        gpu_id = gpu_ids[i % len(gpu_ids)]
        gpu_queues[gpu_id].put(match)
    ...
    # Log distribution
    logger.info(f"📊 Work pre-distributed:")
    for gpu_id in gpu_ids:
        logger.info(f"   GPU {gpu_id} queue: {gpu_queues[gpu_id].qsize()} items")
    
    return gpu_queues

def enhanced_worker_run(self):
    """REPLACE your DualGPUWorker.run() method with this"""
    
    self.logger.info(f"🔥 Worker {self.worker_id} starting on GPU {self.gpu_id}")
    
    # Initialize GPU with explicit verification
    ...
    # Verify GPU context every few operations
    context_check_counter = 0
    
    # Main processing loop
    while self.is_running:
        try:
            # Get work with longer timeout for better queue handling
            try:
                work_item = self.work_queue.get(timeout=10)  # Increased from 5 to 10
            except queue.Empty:
                # Check if this GPU is falling behind
                current_time = time.time()
                self.logger.debug(f"Worker {self.worker_id}: Queue timeout, checking status...")
                continue
            ...
            # Periodic GPU context verification (every 5 operations)
            context_check_counter += 1
            if context_check_counter % 5 == 0:
                try:
                    cp.cuda.Device(self.gpu_id).use()
                    actual_gpu = cp.cuda.Device().id
                    if actual_gpu != self.gpu_id:
                        self.logger.warning(f"⚠️  Worker {self.worker_id}: GPU context drift! "
                                          f"Expected {self.gpu_id}, got {actual_gpu}")
                        # Force context reset
                        cp.cuda.Device(self.gpu_id).use()
                except Exception as e:
                    self.logger.debug(f"GPU context check error: {e}")
            ...
            result['actual_processing_gpu'] = cp.cuda.Device().id if cp.cuda.is_available() else -1
            
            self.result_queue.put(result)
            self.processed += 1
            
            # Enhanced progress logging with GPU verification
            if self.processed % 3 == 0:
                actual_gpu = cp.cuda.Device().id if cp.cuda.is_available() else -1
                ...
                self.logger.info(f"🔥 Worker {self.worker_id}: {self.processed} processed "
                               f"(GPU: {actual_gpu}, Queue: {queue_size}, "
                               f"Time: {processing_time:.1f}s, Mem: {gpu_mem:.1f}MB)")
...
# REPLACE your main() worker creation section with this:
def improved_main_worker_section():
    """
    REPLACE the worker creation section in your main() function with this code
    """
    
    # Create balanced GPU queues instead of single shared queue
    gpu_queues = create_balanced_gpu_queues(all_matches, [0, 1])
    result_queue = queue.Queue()
    
    # Create workers with dedicated GPU queues
    workers = []
    worker_threads = []
    
    logger.info(f"🔥 Starting {args.workers_per_gpu * 2} workers with balanced queues...")
    
    for gpu_id in [0, 1]:
        # Get the pre-balanced queue for this GPU
        gpu_work_queue = gpu_queues[gpu_id]
        
        for worker_idx in range(args.workers_per_gpu):
            worker_id = f"GPU{gpu_id}_W{worker_idx}"
            
            # Create worker with dedicated GPU queue
            worker = DualGPUWorker(worker_id, gpu_id, gpu_work_queue, result_queue, args.gpu_memory)
            ...
            # IMPORTANT: Replace the worker's run method with enhanced version
            worker.run = lambda w=worker: enhanced_worker_run(w)
    ...
    # Wait for worker initialization with verification
    logger.info("⏳ Waiting for worker initialization and verification...")
    time.sleep(5)  # Increased from 3 to 5 seconds
    
    # Test GPU assignment by checking active GPU contexts
    gpu_test_results = {0: 0, 1: 0}
    for worker in workers:
        try:
            # This is a simple test - in practice you'd need to query the worker's current GPU
            assigned_gpu = worker.gpu_id
            gpu_test_results[assigned_gpu] += 1
        except:
            pass
    
    logger.info(f"🔍 Worker GPU assignment: GPU 0: {gpu_test_results[0]}, GPU 1: {gpu_test_results[1]}")
    
    return workers, worker_threads, result_queue

# ADD this enhanced result collection to replace your existing collection loop:
def enhanced_result_collection(all_matches, result_queue, workers):
    """Enhanced result collection with GPU monitoring"""
    
    results = []
    start_time = time.time()
    last_update = start_time
    gpu_activity = {0: time.time(), 1: time.time()}
    
    logger.info(f"🚀 Processing {len(all_matches)} matches with enhanced monitoring...")
    ...
    for i in range(len(all_matches)):
        try:
            ...
            # Track GPU activity
            gpu_used = result.get('actual_processing_gpu', result.get('gpu_id', -1))
            if gpu_used in gpu_activity:
                gpu_activity[gpu_used] = time.time()
            
            current_time = time.time()
            
            # Enhanced progress updates every 5 items or 30 seconds
            if (i + 1) % 5 == 0 or current_time - last_update > 30:
                elapsed = current_time - start_time
                rate = (i + 1) / elapsed if elapsed > 0 else 0
                
                # Count GPU usage
                gpu0_count = sum(1 for r in results if r.get('actual_processing_gpu', r.get('gpu_id', -1)) == 0)
                gpu1_count = sum(1 for r in results if r.get('actual_processing_gpu', r.get('gpu_id', -1)) == 1)
                success_count = sum(1 for r in results if r.get('temporal_offset_seconds') is not None)
                
                # Check for idle GPUs
                gpu0_idle = current_time - gpu_activity[0]
                gpu1_idle = current_time - gpu_activity[1]
                
                gpu0_status = "🟢 ACTIVE" if gpu0_idle < 30 else f"🔴 IDLE ({gpu0_idle:.0f}s)"
                gpu1_status = "🟢 ACTIVE" if gpu1_idle < 30 else f"🔴 IDLE ({gpu1_idle:.0f}s)"
                
                logger.info(f"📊 Progress: {i+1}/{len(all_matches)} ({rate:.2f}/s) | "
                           f"GPU0: {gpu0_count} {gpu0_status} | "
                           f"GPU1: {gpu1_count} {gpu1_status} | "
                           f"Success: {success_count}")
                
                # Alert if GPU 1 is idle for too long
                if gpu1_idle > 60:  # 1 minute idle
                    logger.warning(f"⚠️  GPU 1 has been idle for {gpu1_idle:.0f} seconds!")
            ...
            # Check worker status
            active_workers = sum(1 for w in workers if getattr(w, 'is_running', False))
...
# ADD this final diagnosis function:
def final_gpu_diagnosis(results):
    """Final diagnosis of GPU usage patterns"""
    
    print("\n" + "="*80)
    print("🔍 FINAL GPU UTILIZATION ANALYSIS")
    print("="*80)
    
    gpu0_results = [r for r in results if r.get('actual_processing_gpu', r.get('gpu_id', -1)) == 0]
    gpu1_results = [r for r in results if r.get('actual_processing_gpu', r.get('gpu_id', -1)) == 1]
    unknown_results = [r for r in results if r.get('actual_processing_gpu', r.get('gpu_id', -1)) not in [0, 1]]
    
    print(f"📊 GPU 0 processed: {len(gpu0_results)} matches")
    print(f"📊 GPU 1 processed: {len(gpu1_results)} matches")
    print(f"❓ Unknown GPU: {len(unknown_results)} matches")
    
    if len(gpu1_results) == 0:
        print("❌ CRITICAL ISSUE: GPU 1 processed ZERO matches!")
        print("🔧 SOLUTIONS:")
        print("   1. Reduce --workers-per-gpu to 1")
        print("   2. Add time.sleep(0.1) between worker starts")
        print("   3. Check if specific video types are causing GPU 0 to dominate")
        print("   4. Try --gpu-memory 8.0 to reduce memory pressure")
    elif abs(len(gpu0_results) - len(gpu1_results)) > len(results) * 0.3:
        balance_ratio = min(len(gpu0_results), len(gpu1_results)) / max(len(gpu0_results), len(gpu1_results)) * 100
        print(f"⚠️  Load imbalance detected: {balance_ratio:.1f}% balance")
        print("🔧 Try adjusting queue timeout or processing parameters")
    else:
        balance_ratio = min(len(gpu0_results), len(gpu1_results)) / max(len(gpu0_results), len(gpu1_results)) * 100
        print(f"✅ Good load balance: {balance_ratio:.1f}%")
    
    # Analyze processing times
    gpu0_times = [r.get('processing_time_seconds', 0) for r in gpu0_results if r.get('processing_time_seconds')]
    gpu1_times = [r.get('processing_time_seconds', 0) for r in gpu1_results if r.get('processing_time_seconds')]
    
    if gpu0_times:
        print(f"⏱️  GPU 0 avg time: {sum(gpu0_times)/len(gpu0_times):.2f}s")
    if gpu1_times:
        print(f"⏱️  GPU 1 avg time: {sum(gpu1_times)/len(gpu1_times):.2f}s")
...
def main():
    ...
    results = enhanced_result_collection(all_matches, result_queue, workers)
    final_gpu_diagnosis(results)
