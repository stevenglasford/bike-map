...
def create_balanced_gpu_queues(all_matches, gpu_ids=[0, 1]):
    ...
    # Log distribution - using print instead of logger to avoid NameError
    print(f"📊 Work pre-distributed:")
    for gpu_id in gpu_ids:
        print(f"   GPU {gpu_id} queue: {gpu_queues[gpu_id].qsize()} items")
    ...
    def run(self):
        ...
        while self.is_running:
            try:
                ...
                try:
                    work_item = self.work_queue.get(timeout=10)  # Increased timeout for balanced queues
                except queue.Empty:
                    continue
                ...
                # Add processing metadata for diagnosis
                ...
                if self.processed % 5 == 0:
                    ...
                    self.logger.info(f"🔥 Worker {self.worker_id}: {self.processed} processed "
                                   f"(GPU: {actual_gpu}, Time: {processing_time:.1f}s, Mem: {gpu_mem:.1f}MB)")
...
def main():
    ...
    # FIXED: Setup dual GPU processing with balanced queues
    ...
    for i in range(len(all_matches)):
        try:
            ...
            if (i + 1) % 5 == 0 or current_time - last_update > 30:
                ...
                gpu0_count = sum(1 for r in results if r.get('actual_processing_gpu', r.get('gpu_id')) == 0)
                gpu1_count = sum(1 for r in results if r.get('actual_processing_gpu', r.get('gpu_id')) == 1)
    ...
    for _ in range(total_workers):
        # Send shutdown signals to each GPU queue
        for gpu_id in [0, 1]:
            try:
                gpu_queues[gpu_id].put(None)
            except:
