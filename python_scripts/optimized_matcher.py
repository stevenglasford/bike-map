...
class GPUVideoProcessor:
    ...
    async def _process_video_frames(self, cap, gpu_id: int, sample_interval: int, 
        ...
        with cp.cuda.Device(gpu_id):
            ...
            for frame_idx in range(0, frame_count, sample_interval):
                ...
                try:
                    ...
                    if len(batch_frames) >= self.batch_size or frame_idx >= frame_count - sample_interval:
                        if batch_frames:
                            # Upload to GPU
                            frames_gpu = cp.asarray(cp.array(batch_frames), stream=self.streams[gpu_id][0])
            ...
            for stream in self.streams[gpu_id]:
                stream.synchronize()
...
class GPXProcessor:
    ...
    def __init__(self, gpu_id: int = 0, chunk_size: int = 1000):
        ...
        if gpu_id >= cp.cuda.runtime.getDeviceCount():
            logger.error(f"GPU {gpu_id} not available for GPX processing, using CPU")
            exit(69)
            self.use_gpu = False
        else:
            self.use_gpu = True
    ...
    def _calculate_features_gpu(self, times: cp.ndarray, lats: cp.ndarray, 
        ...
        with cp.cuda.Device(self.gpu_id):
            # Upload to GPU
            times_gpu = cp.asarray(times, stream=self.stream)
            lats_gpu = cp.asarray(lats, stream=self.stream)
            lons_gpu = cp.asarray(lons, stream=self.stream)
            eles_gpu = cp.asarray(eles, stream=self.stream)
